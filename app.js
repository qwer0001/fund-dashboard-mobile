...
async function sbFetch(path,{method='GET',body=null,auth=true,retry=true}={}){if(auth)await validSession();const headers={apikey:SB_KEY};if(auth&&state.session?.accessToken)headers.Authorization=`Bearer ${state.session.accessToken}`;if(body!==null)headers['Content-Type']='application/json';let r;try{r=await fetch(`${SB_URL}${path}`,{method,headers,cache:'no-store',...((body!==null)?{body:JSON.stringify(body)}:{})});}catch(_){throw new Error('无法连接私密云端，请检查网络或 VPN');}if(r.status===401&&auth&&retry){await refreshSession();return sbFetch(path,{method,body,auth,retry:false});}const text=await r.text();let data=null;if(text){try{data=JSON.parse(text);}catch(_){data=text;}}if(!r.ok){const msg=(data&&typeof data==='object'&&(data.message||data.msg||data.error_description||data.error))||String(data||`HTTP ${r.status}`);const e=new Error(msg);e.code=String(msg).includes('revision_conflict')?'revision_conflict':`http_${r.status}`;throw e;}return data;}
...
async function loadCloud(){try{const[c,h]=await Promise.all([fetchCurrent(),fetchHistory()]);state.current=c;state.history=h;state.cores=ledgerCores(c);saveCache();render();return true;}catch(e){if(state.current){render();setCloudStatus(`离线缓存 · ${e.message}`,'bad');return false;}throw e;}}
...
function rangeStart(range){const d=new Date();if(range==='1m')d.setMonth(d.getMonth()-1);else if(range==='3m')d.setMonth(d.getMonth()-3);else if(range==='6m')d.setMonth(d.getMonth()-6);else if(range==='1y')d.setFullYear(d.getFullYear()-1);else if(range==='3y')d.setFullYear(d.getFullYear()-3);else return 0;return d.getTime();}
